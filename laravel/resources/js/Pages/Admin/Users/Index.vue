  ...
  <AuthenticatedLayout>
    ...
    <v-card class="pa-4">
      <div class="d-flex flex-wrap align-center">
        ...
        <v-spacer />
        <Link 
          :href="route('admin.users.invite.create')" as="div"
          v-if="can('users all create')"
        >
      ...
      >
        ...
        <template #[`item.role_name`]="{ item }">
          <v-chip :color="item.role_color" variant="flat"
          >{{ item.role_name }}</v-chip>
        </template>
        <template #[`item.action`]="{ item }">
          <Link 
            :href="route('admin.users.edit', item.id)" as="button"
            v-if="can('users all update')"
          >
