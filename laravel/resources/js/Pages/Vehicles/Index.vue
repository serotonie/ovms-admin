  ...
  <AuthenticatedLayout>
    ...
    <v-card class="pa-4">
      <div class="d-flex flex-wrap align-center">
        ...
        <v-spacer />
        <Link 
          :href="route('admin.vehicles.create')" as="div"
          v-if="can('vehicles all create')"
        >
      ...
      >
        ...
        <template #[`item.action`]="{ item }">
          <Link 
            :href="route('admin.vehicles.edit', item.id)" as="button"
            v-if="can('vehicles all update')"
          >
            <v-icon color="warning" icon="mdi-pencil" size="small" />
          </Link>
          <v-icon 
            class="ml-2" color="error" icon="mdi-delete" size="small" @click="deleteItem(item)" 
            v-if="can('vehicles all delete')"
          />
          <v-icon 
            class="ml-2" color="werning" icon="mdi-restore" size="small" @click="resetItem(item)" 
